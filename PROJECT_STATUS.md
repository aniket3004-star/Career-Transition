...
- GitHub Actions run #34 (`35484034198`) completed successfully on commit `51a75f347090874ac4095d09a5df410d7d65e0ac`, verifying the current CLI fixture fix and the repository test suite at that commit.
...
3. Obtain green CI evidence on the exact current candidate commit; run #34 verifies `51a75f3`, but this status-document update creates a newer candidate commit that must itself pass CI before it can be the final acceptance SHA.
